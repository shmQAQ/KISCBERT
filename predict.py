...
def main(args):
    ...
    for fold, (train_index, val_index) in enumerate(kfold.split(dataset)):
        ...
        train_loader = DataLoader(dataset, batch_size=args.batch_size, sampler=train_sampler, collate_fn=dataset.collate_fn, num_workers=4, pin_memory=True,persistent_workers=True)
        val_loader = DataLoader(dataset, batch_size=args.batch_size, sampler=val_sampler, collate_fn=dataset.collate_fn, num_workers=4, pin_memory=True,persistent_workers=True)
