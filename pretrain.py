...
def train_step(x, adjoin_matrix, y, char_weight,model, optimizer,device,loss_function=CrossEntropyLoss(reduction='none')):
    seq = (x == 0).float()
    ...
    train_accuracy = torch.argmax(predictions, dim=-1).eq(y).sum().item() / y.numel()
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for x, y,adjoin_matrix,weights in train_loader:
            ...
            train_loss, train_accuracy = train_step(x=x, adjoin_matrix=adjoin_matrix, y=y, char_weight=weights,model=model, optimizer=optimizer,device=device)
